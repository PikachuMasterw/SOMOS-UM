...
(function (global, document) {
  ...
  const REQUEST_ENDPOINT = VERCEL_BASE_URL + "/gemini-proxy";
  ...
  const JoaoIA = {
    ...
    addMessage: function (text, isUser = false) {
      ...
      messageDiv.className = `joao-ia-message ${
        isUser ? "joao-ia-user-message" : "joao-ia-bot-message"
      }`;
    ...
    sendToBackend: async function (userMessage) {
      ...
      try {
        ...
        if (data.status === "success" && data.resposta) {
          ...
          // Se a resposta terminar abruptamente, adicionar "..."
